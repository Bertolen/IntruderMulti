...
void ALobbyPC::SaveGame()
{
	UE_LOG(IntruderDebug, Verbose, TEXT("SaveGame - Begin"));
	if (!PlayerSaveGameRef) {
		PlayerSaveGameRef = Cast<UPlayerSaveGame>(UGameplayStatics::CreateSaveGameObject(UPlayerSaveGame::StaticClass()));
	}

	PlayerSaveGameRef->S_PlayerInfo = PlayerSettings;

	UGameplayStatics::SaveGameToSlot(PlayerSaveGameRef, PlayerSettingsSave, 0);
	UE_LOG(IntruderDebug, Verbose, TEXT("SaveGame - End"));
}
